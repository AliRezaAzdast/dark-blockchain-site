            ...
            <div className="my-container">
                    ...
                    <div className="flex flex-none gap-8">
                        {cardData.map(({ id, image, title, description, color }) => (
                            <div className="relative z-0 p-8 md:p-10 max-w-xs md:max-w-md group">
                                <div key={id} className={twMerge("absolute size-16 rounded-xl bg-fuchsia-500 top-1.5 right-1.5 -z-10 blur-lg opacity-0 group-hover:opacity-100 transition duration-300", color === 'lime' && 'bg-lime-500', color === "cyan" && 'bg-cyan-500', color === 'violet' && 'bg-violet-500')}></div>
                ...
                <div className="flex justify-center mt-10 ">
                    <div className="bg-zinc-950 inline-flex gap-4 p-2.5 rounded-full">
                        {cardData.map(({title}) => (
                            <div key={title} className="size-2.5 bg-zinc-500 cursor-pointer rounded-full">
